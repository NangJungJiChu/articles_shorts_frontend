...
<template>
  <article ref="cardRef" class="shorts-card" :class="{ 'is-expanded': isExpanded, 'is-overflowing': isOverflowing }"
    @click="toggleExpand">
    <!-- 1. Content Area -->
    ...
    <div class="actions-container" @click.stop>
      ...
      <button class="action-btn" @click="handleLike">
        <Icon class="icon" name="favorite" :type="localIsLiked ? 'filled' : 'outlined'" />
...
.shorts-content-wrapper {
  height: 100%;
  padding: 24px 16px;
  padding-bottom: calc(58px + 104px);
  /* Space for expansion overlay & interaction padding */
  overflow: hidden;
